...
public class HiveTableLineageParserBriefTemp11 {
    /*
     * TableRelation
     */
    ...
    private static void parseCurrentNode(ASTNode ast) {
        if (ast.getToken() != null) {
            ...
            case HiveParser.TOK_JOIN:
                ...
                if (ast.getChild(0).getType() == HiveParser.TOK_LEFTOUTERJOIN||ast.getChild(0).getType() == HiveParser.TOK_JOIN) {
                    ...
                    if (null != tableAliasMap.get(currentTableAliasName)&&!tableAliasMap.get(currentTableAliasName).contains("TOK_QUERY")) {
                ...
                } else if(ast.getChild(0).getType() == HiveParser.TOK_SUBQUERY){
                    while (joinTopLevelTableStackTemp.size() > 0) {
                        joinTopLevelTableStackTemp.pop();
                    }
            ...
            case HiveParser.TOK_SUBQUERY:
                if (ast.getChildCount() == 2) {
                    ...
                    if (tokDbNameStack.size() > 0) {
                        ...
                        if (tokDBAliasName.equalsIgnoreCase("tok_query")||tokDBAliasName.equals("")) {
                            tokDbNameStack.pop();
                            ...
                            } while (tokDbNameStack.size() > 0 && !tokDbNameStack.peek().equalsIgnoreCase("tok_query")&&!tokDbNameStack.peek().equals(""));
    ...
    public static void main(String[] args) throws IOException {
        ...
        HiveTableLineageParserBriefTemp11.parse(ast);
        ...
        Set<String> referTableFieldSet = new HashSet<String>();
        for (Entry<String, String> e : insertSelectFieldMap.entrySet()) {
            System.out.println("key=" + e.getKey());
...
//					System.out.println("\t-------- i1 --------");
//					System.out.println("\t" + i1);
                    Set<String> firstLevelTableAliasSet = SqlLineageUtil.transformTo1stLevelAliasTableName(i1,
                            flattenedTopLevelTableAliasMap);
                    if (null != firstLevelTableAliasSet) {
                        for (String i2 : firstLevelTableAliasSet) {
//							System.out.println("\t\t-------- i2 --------");
//							System.out.println("\t\t" + i2);
                            ...
                            if (null != s3) {
                                for (String i3 : s3) {
//									System.out.println("\t\t\t-------- i3 --------");
//									System.out.println("\t\t\t" + SqlLineageUtil.replaceWithSrcTableName(i3, tableAliasLineageMap));
                                    referTableFieldSet
                                            .add(SqlLineageUtil.replaceWithSrcTableName(i3, tableAliasLineageMap));
                                }
                            }
                        }
                    }
        ...
        JamesUtil.printDivider("referTableFieldSet");
        JamesUtil.printSet(referTableFieldSet);
    }
}