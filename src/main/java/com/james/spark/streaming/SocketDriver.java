...
public class SocketDriver extends AbstractDriver {
	...
	public void init() throws Exception {
		...
		System.out.println(String.format("Waiting for client to connect on port %d", port));
		SocketChannel socketChan = socketStream.init();
		System.out.println(String.format("Client %s connected on port %d", socketChan.getRemoteAddress(), port));
	...
	static class SocketStream extends Thread {
		...
		public SocketChannel init() throws IOException {
			...
			System.out.println(String.format("Listening on %s", server.getLocalAddress()));
			return server.accept();
		}

		public void kickOff(SocketChannel socket) {
			System.out.println("Kicking off data transfer");
			this.socket = socket;
		}
		...
		public void close() throws IOException {
			...
			System.out.println(String.format("SocketStream is closing after writing %d bytes and %d lines", totalBytes,
					totalLines));
		}
	}
