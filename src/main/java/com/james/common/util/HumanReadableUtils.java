...
public class HumanReadableUtils {
     ...
     * 操作系统中每个单位含有的字节数
     */
    private static final double[] BYTE_SIZE_HD = { 1, 1000, 1000 * 1000, 1000 * 1000 * 1000,
            1000d * 1000 * 1000 * 1000 };
    /**
     * 
     */
    private static final double[] BYTE_SIZE_THRESHOLD = { 1, 999, 999 * 1024, 999 * 1024 * 1024,
            999d * 1024 * 1024 * 1024 };
    /**
     * 
     */
     ...
     * 每种时间单位对应的毫秒数
     */
    private static final long[] TIME_SPAN = { 1, 1000, 1000 * 60, 1000 * 60 * 60, 1000 * 60 * 60 * 24,
            1000l * 60 * 60 * 24 * 365 };
    /**
     * 时间单位(中文)
     */
    ...
    public static String byteSizeWithoutUnit(long bytes, boolean byHDStandard) {
        if (bytes < 0) {
            return NA;
        }
        double[] byte_size = BYTE_SIZE;
        ...
        while (--i >= 0) {
            if (bytes >= byte_size_threadshold[i]) {
                return df.format((bytes / byte_size[i]));
            }
        }
        return "";
    }
